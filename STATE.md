...
## 2026-08-16: 一覧表（feature table）のヘッダーと行が重なる不具合

**担当**: Claude Code（Sonnet 5）

ユーザーが実際に生成したHTML（用途地域レイヤー、A29_003〜A29_005列、1386件）の
スクリーンショットで、左下の一覧表のヘッダー行と直下の行のテキストが重なって
見える不具合を報告。

**原因**: `point-list.js` の列幅自動計算が `文字数 * 9px` という決め打ちで
テキスト幅を見積もっていた。この9px/文字はASCII文字（列名 `A29_005` など）向けの
値で、全角の日本語テキスト（値の `第二種住居地域` など）は実際には1文字あたり
もっと幅を取るため、列幅が実際の描画幅より狭く計算されていた。結果として
セルのテキストが折り返され、`.fag-table-row` に `height:30px` はあっても
`overflow` の指定が無かったため、はみ出した2行目が固定30px幅を突き破って
下の行の上に重なって表示されていた。

**修正**:
1. `point-list.js`: 文字数ベースの見積もりを `ctx.measureText`（`label-layer.js`
   と同じ手法）による実測に置き換え（`fagTableTextWidth`）。ヘッダーは
   実際に太字で表示される（`.fag-table-row-head` が `font-weight:700`）ため
   bold指定で測定、値は通常フォントで測定。フォントは初回ロード時に
   `#feature-table-scroll` の computed style から遅延取得するので、
   テーマのカスタムフォント（`main.js::applyTheme`）にも追従する。
2. `style.css`: `.fag-table-row` に `overflow: hidden` を追加（保険 — 何らかの
   理由でセル内容が30pxを超えても、下の行に漏れず自分の枠内でクリップされる）。
   `.fag-table-head-label` に `min-width: 0` を追加（flexアイテムの既定
   `min-width:auto` によってellipsis省略が効かなくなるケースへの保険）。

**検証**: `node --check` のみ（QGIS未起動・実データでの見た目確認はまだ）。
次回QGISで実際に一覧表を開いて、列幅が日本語テキストに対して適切か、
ヘッダーと行が重ならないかを確認すること。

**同日フォローアップ**: 上記の保険で追加した `.fag-table-row { overflow:
hidden }` が原因で、横スクロールバーが消える回帰をユーザーが実際の生成
HTMLで発見・報告。原因: `.fag-table-row` は幅 `auto`（パネル幅いっぱいに
収まる）のブロックボックスだが、中の `.fag-table-cell` は固定px幅の
flexアイテム（`flex:0 0 auto`）で縮まないため、列幅の合計がパネル幅を
超えると本来は行の外にはみ出して描画され、それを祖先の `#feature-table-
scroll`（`overflow:auto`）が「スクロール可能な範囲」として検知して横
スクロールバーを出す仕組みだった。行に `overflow:hidden` を付けたことで
そのはみ出し自体を行の内側でクリップしてしまい、`#feature-table-scroll`
からは「はみ出しなし」に見えるため横スクロールバーが出なくなり、かつ
はみ出した列は単純にアクセス不能（非表示）になっていた。point-list.js
側の実測幅修正で本来の重なり原因（セルの折り返し）は解消済みのため、
この保険のCSSは不要と判断し `.fag-table-row` から `overflow:hidden` を
削除して撤回（`.fag-table-head-label` の `min-width:0` は行の外形幅に
影響しないためそのまま維持）。

**同日: モバイル表示の総点検（ユーザーが選択した改善方向）**。ヘッドレス
Edgeでモバイル幅を検証しようとしたところ、既存のEdgeセッションに接続
される都合で`--window-size`が最小約504pxに固定される制約を発見
（`--user-data-dir`を分けても解消せず）。`<iframe width:390px>`で
包んだラッパーページを別途用意し、その中で実ページを読み込むことで
真の390×844相当のビューポートを再現する方法で回避・検証した。

見つけた問題:
1. **（重大・修正済み）一覧表を開くと地図がほぼ全部隠れる**:
   `#feature-table-panel`の`max-height: calc(100vh - 76px)`は
   デスクトップ（幅360pxの隅パネル）では問題ないが、モバイルでは
   `max-width: calc(100vw - 20px)`によりほぼ全幅にもなるため、
   「ほぼ全高×ほぼ全幅」＝開くと地図がほぼ完全に隠れていた。
   `@media (max-width:768px)`に`#app.responsive #feature-table-panel
   { max-height: 55vh; }`を追加し、地図が約45%残るように修正
   （iframe法で修正前後のスクリーンショット比較により確認済み）。
2. **（修正済み）選択/エクスポートバーが一覧表を閉じた状態でも常に
   表示される**: `#selection-bar`はcolumn-reverseレイアウトの最初の
   子要素として「一覧表が閉じていても触れるように」意図的に常時表示に
   してある（v0.4.0 item 9の仕様、デスクトップでは隅の小さいパネル
   なので気にならない）。ユーザーに確認したところ「モバイルのみ
   一覧表と一緒に折りたたむ」を選択（デスクトップの挙動は変えない）。
   `@media (max-width:768px)`に`#app.responsive #feature-table-
   panel.fag-collapsed #selection-bar { display: none; }`を追加し、
   iframe法で折りたたみ時/展開時の両方を確認済み（折りたたみ時は
   選択バーが消えて地図がさらに広く見える。展開時は`.fag-collapsed`
   が外れるので元通り表示される）。

（Layer PanelがモバイルでLaye...と見切れる、という最初の見立ては
`--window-size`が意図通りに反映されていなかった検証環境側の見かけ上の
不具合と判明・誤り。iframe法での再検証では正しく全体表示された。）

**さらに同日フォローアップ: 横スクロールバーは戻ったが、重なりも再発**。
`point-list.js`の実測幅修正は列幅の見積もり自体は正しく直していたが、
重なりの本当の原因は「セルの折り返し」ではなく別のCSS構造バグだった。
`.fag-table-row`は`display:flex`だがブロックレベルの外側表示のため、
`width:auto`は「中の子要素の合計幅」ではなく「親（`#feature-table-
scroll`、パネル幅≒360px）いっぱい」に決まる。中の`.fag-table-cell`は
`flex:0 0 auto`（縮まない固定px幅）なので、列が多くて合計幅が360pxを
超えると、はみ出した列は行自身の**箱の外側**に描画される
（`overflow:visible`のデフォルト挙動。これが元々横スクロールバーが
出ていた仕組み ―― 祖先の`overflow:auto`がこの「はみ出した描画」を
スクロール可能領域として拾っていた）。ところが`.fag-table-row-head`の
不透明な背景色（`background: var(--panel-bg)`）は行**自身の箱**にしか
塗られない。つまり横スクロールで360pxを超えた列を表示した瞬間、その
列のヘッダー部分は「背景の外」＝透明になり、真下にスクロールしてきた
データ行がそのまま透けて見える ―― これがユーザー報告の「ヘッダーと
行が重なって見える」の正体（画面外に隠れているのではなく、本当に
透明な背景を通して重なって描画されていた）。ヘッダーだけでなく
データ行側の`.fag-row-selected`/hoverハイライトも同じ理由で360pxを
超えた列に色が付かない、という別の不具合も理論上抱えていた。

**修正**: `.fag-table-row`に`width: max-content; min-width: 100%;`を追加
（`style.css`）。行自身の箱を実際の列合計幅まで広げつつ、内容が
パネル幅より狭いときは今まで通りパネル幅いっぱいに広がる
（`min-width:100%`が下限として効く）。これで背景色が横スクロール位置に
関係なく行全体を覆うようになり、横スクロールバーも
（見た目上の重なりではなく実際に箱が広がったことで、より確実に）
機能する。header/dataどちらの行も同じクラスなので、選択/hover
ハイライトの塗り漏れも同時に直る想定。

**検証（実施済み・確認できた）**: 静的なCSS読解だけで2回連続「直った」と
誤報告した反省を踏まえ、今回は実際にヘッドレスEdge（`msedge.exe
--headless=new --screenshot`）でユーザーの実際の生成ファイル
（`map to html test3.html`）のコピーを開き、同じ横スクロール位置
（`scrollLeft=260`）を再現してスクリーンショット比較した。
修正前のCSS（`width`指定なし）では報告通りヘッダー行の上に
データ値（「近隣商業地域 80」）が透けて重なって見えることを再現・確認、
`width: max-content; min-width: 100%;` を当てたコピーでは同じ操作でも
重なりが消え、ヘッダー行が単独できれいに表示されることを確認した
（`--dump-dom`で実際のDOM/inline styleも確認: `#feature-table-rows`の
`transform: translateY(...)`計算自体は元々正しく、バグは本当に
`.fag-table-row`の背景がボックス幅を超えて塗られない点だけだった）。
